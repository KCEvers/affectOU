...
After affect is perturbed, it takes time to return to its baseline. The speed of this return is determined by $\mathbf{\Theta}$, which characterizes how long the effect of perturbations persists. This persistence can be quantified through the relaxation time $\tau$, which is the time for the expected deviation to shrink to $1/e \approx 36.8\%$. Effects decay exponentially: at two $\tau$, about 13.5\% remains; at three $\tau$, about 5\%. $\tau$ is also known as the characteristic time scale, which can also be defined as the half-life $t_{1/2} = \ln 2 \cdot \tau$, which is the time for the expected deviation to shrink to 50\%. 

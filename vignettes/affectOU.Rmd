...
The OU framework extends naturally to model multiple affect dimensions (e.g., valence and arousal). For a d-dimensional process:

$$d\mathbf{X}(t) = \mathbf{\Theta}(\mathbf{\mu} - \mathbf{X}(t))dt + \mathbf{\Gamma} \, d\mathbf{W}(t)$$

By default, `affectOU()` creates a model with uncoupled dynamics (diagonal $\mathbf{\Theta}$ and $\mathbf{\Gamma}$) and identical parameters across dimensions:
...
Above, we use [`update()`](../reference/update.affectOU.html) to modify an existing model without recreating it from scratch. For coupled dynamics where dimensions influence each other, use a non-diagonal $\mathbf{\Theta}$ matrix:

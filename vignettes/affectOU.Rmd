...
The OU framework extends naturally to multiple affect dimensions (e.g., valence and arousal, positive affect and negative affect,...). For a d-dimensional process, the OU looks like:
...
model_2d <- update(
  ...
  sigma = diag(c(1, 1))      # Different noise levels
)
...
Though the OU is a continous-time process, it can be simulated only through discretization. In the package, we use Euler-Maruyama approximation, a common numerical approach for simulating stochastic differential equations. For the OU, it amounts to updating the state $\mathbf{X}$ after a discrete time-step $\Delta t$ according to the following equation:
